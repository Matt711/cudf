...
inline void log_exception_helper(const std::source_location& /*loc*/)
{
  try {
    ...
    SIRIUS_LOG_ERROR("Exception caught: {}", e.what());
  } catch (...) {
    SIRIUS_LOG_ERROR("UNKNOWN exception caught");
  }
}

template <typename... Args>
void log_exception_helper(const std::source_location& /*loc*/,
                          ...
                          Args&&... args)
{
  ...
  try {
    ...
    SIRIUS_LOG_ERROR("{}: {}", user_msg, e.what());
  } catch (...) {
    SIRIUS_LOG_ERROR("{}: UNKNOWN exception", user_msg);
  }
}
