...
class uring_reactor {
 public:
  ...
  class reactor_context {
   public:
    reactor_context(config cfg, cudf_streaming::prefetch::memory::fixed_size_host_memory_resource* mr)
      : _config(cfg), _mr(mr)
    {
    }

    [[nodiscard]] const config& cfg() const noexcept { return _config; }
    [[nodiscard]] cudf_streaming::prefetch::memory::fixed_size_host_memory_resource* host_memory_resource()
      const noexcept
    {
      return _mr;
    }
   ...
   private:
    config _config;
    cudf_streaming::prefetch::memory::fixed_size_host_memory_resource* _mr{nullptr};
  };
 ...
 private:
  ...
  std::string _tname;
  cudf_streaming::prefetch::memory::fixed_multiple_blocks_allocation _bounce_storage;
  std::size_t _bounce_slot_size;
  std::stop_source _stop_source;
  std::jthread _worker;
  moodycamel::BlockingConcurrentQueue<std::unique_ptr<grouped_io_request>> _requests;
