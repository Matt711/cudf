...
// ---------------------------------------------------------------------------
// uring_io_op — one physical IO operation queued to the io_uring worker.
// Wraps the shared io_op_request with uring-specific fields.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// detail — internal helpers shared across the worker loop
// ---------------------------------------------------------------------------

namespace detail {

// Rebuild the iovec list for the bytes that remain after `skip` have landed.
inline void fill_remaining_iovecs(std::vector<iovec> const& src,
                                   std::size_t skip,
                                   std::vector<iovec>& out)
{
  out.clear();
  std::size_t to_skip = skip;
  for (auto const& iov : src) {
    if (to_skip >= iov.iov_len) {
      to_skip -= iov.iov_len;
      continue;
    }
    auto* base = static_cast<std::uint8_t*>(iov.iov_base);
    out.push_back(iovec{base + to_skip, iov.iov_len - to_skip});
    to_skip = 0;
  }
}

// True iff O_DIRECT is both configured and the fd is valid (>= 0).
[[nodiscard]] inline bool odirect_available(bool use_odirect, int fd_direct) noexcept
{
  return use_odirect && fd_direct >= 0;
}

// True iff the range and every iovec satisfy O_DIRECT's 4 KiB alignment rules.
[[nodiscard]] inline bool is_odirect_compatible(range const& rng,
                                                  std::span<iovec const> iovecs) noexcept
{
  if (rng.offset % IO_BLOCK_SIZE != 0 || rng.size % IO_BLOCK_SIZE != 0) return false;
  for (auto const& iov : iovecs) {
    if (reinterpret_cast<std::uintptr_t>(iov.iov_base) % IO_BLOCK_SIZE != 0) return false;
    if (iov.iov_len % IO_BLOCK_SIZE != 0) return false;
  }
  return true;
}

// True iff the errno indicates an O_DIRECT alignment or availability error that
// should trigger a transparent fallback to buffered I/O.
[[nodiscard]] inline bool is_odirect_runtime_error(int errc) noexcept
{
  return errc == EINVAL || errc == EFAULT || errc == ENXIO;
}

// Choose a per-operation byte target based on the current pipeline state.
// Returns 0 when the heuristic defers to the caller's fallback.
...
{
  if (free_slots == 0 || block_size == 0) return 0;
  auto const per_slot = backlog_bytes / (4 * free_slots);
  return (per_slot / block_size) * block_size;
}

}  // namespace detail

// Maximum single-operation IO size used by the dynamic IO planner.
inline constexpr std::size_t max_dynamic_io_size = 256UL * 1024UL * 1024UL;
...
struct io_slot {
  ...
  std::unique_ptr<cudf_streaming::prefetch::cuda::cuda_event> copy_event;
  int event_device{-1};
    ...
    auto start_device_copy = [&](io_slot& slot) noexcept {
      ...
      try {
        ...
        if (slot.copy_event == nullptr || slot.event_device != device) {
          slot.copy_event   = std::make_unique<cudf_streaming::prefetch::cuda::cuda_event>(cudaEventDisableTiming);
...
#endif  // CUDF_STREAMING_HAS_URING
