...
// (cudf_streaming::prefetch::io::uring::request_manager, ...) keep resolving.
using request_manager    = cudf_streaming::prefetch::io::request_manager;
using device_cpy_request = cudf_streaming::prefetch::io::device_cpy_request;
...
struct chunked_rx_request {
  ...
  {
    if (cpy_req) [[likely]] {
      iovec const buf{chunk.data(), chunk.size};
      return cpy_req->copy_async(range{chunk.offset, chunk.size},
                                  std::span<iovec const>{&buf, 1},
                                  event);
    } else {
      return cudaSuccess;
    }
