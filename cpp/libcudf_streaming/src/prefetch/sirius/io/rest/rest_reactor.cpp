...
namespace cudf_streaming::prefetch::io::rest {
...
{
  try {
    auto const parsed = cudf_streaming::prefetch::io::parse(path);
...
void rest_reactor::worker_loop(std::stop_token const& stop_token)
{
  ...
  try {
    ...
    struct parked_copy {
      slot_pool::token token;
      cudf_streaming::prefetch::cuda::cuda_event* event{nullptr};
      std::unique_ptr<rest_io_op_request> req;
    };
    std::unordered_map<int, std::vector<cudf_streaming::prefetch::cuda::cuda_event>> copy_events;
    ...
    auto event_for = [&](int device_id, std::size_t slot_index) {
      ...
      if (events.size() != _config.max_connections) {
        ...
        std::vector<cudf_streaming::prefetch::cuda::cuda_event> initialized;
        initialized.reserve(_config.max_connections);
        std::generate_n(std::back_inserter(initialized), _config.max_connections, [] {
          return cudf_streaming::prefetch::cuda::cuda_event{cudaEventDisableTiming};
        });
    ...
    auto poll_copy_completions = [&] {
      using query_result = cudf_streaming::prefetch::cuda::event::query_result;
    ...
    auto allocate_staging = [&](rest_io_op_request& request) {
      ...
      using allocation_type =
        cudf_streaming::prefetch::memory::fixed_size_host_memory_resource::multiple_blocks_allocation;
