...
using multiple_blocks_allocation =
  typename cudf_streaming::prefetch::memory::fixed_size_host_memory_resource::multiple_blocks_allocation;

buffer_pool::buffer_pool(cudf_streaming::prefetch::memory::memory_reservation_manager& reservation_manager,
                         double reservation_fraction_for_prefetching,
                         double max_prefetching_budget_fraction)
{
  auto host_mrs = reservation_manager.get_memory_spaces_for_tier(cudf_streaming::prefetch::memory::Tier::HOST);
  if (host_mrs.empty()) {
  ...
  _chunk_bytes =
    host_mrs.front()->get_memory_resource_of<cudf_streaming::prefetch::memory::Tier::HOST>()->get_block_size();

  std::for_each(host_mrs.begin(), host_mrs.end(), [&](auto* mr) {
    auto* mmr = const_cast<cudf_streaming::prefetch::memory::memory_space*>(mr);
    ...
    _host_arenas.push_back(
      host_arena{mr->get_device_id(),
                 ...
                 mmr->get_memory_resource_of<cudf_streaming::prefetch::memory::Tier::HOST>()});
