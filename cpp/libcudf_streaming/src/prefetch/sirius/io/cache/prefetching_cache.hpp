...
using request_queue_type = moodycamel::BlockingConcurrentQueue<cache_request>;
...
class prefetching_cache {
  ...
  friend class cudf_streaming::prefetch::io::sirius_datasource;
  friend class prefetching_handle;

 public:
  using byte_range = cudf::io::text::byte_range_info;

  prefetching_cache(cudf_streaming::prefetch::memory::memory_reservation_manager& reservation_manager,
                    ioctx* io_ctx,
                    const config& cfg,
                    std::shared_ptr<const cudf_streaming::prefetch::memory::topology_index> topology_index);
  ~prefetching_cache();
 ...
 private:
  ...
  void retire_pins_after_stream(rmm::cuda_stream_view stream,
                                cudf_streaming::prefetch::cuda::device_copy_batch const& copies,
  ...
  std::shared_ptr<const cudf_streaming::prefetch::memory::topology_index> const _topology_index;
