...
struct device_cpy_request {
                                       ...
                                       cudaEvent_t event = nullptr) const noexcept
  {
    try {
      ...
      cudf_streaming::prefetch::cuda::device_copy_batch batch;
...
// ---------------------------------------------------------------------------
// request_manager — fan-in coordinator alias used by reactor-layer request types.
// ---------------------------------------------------------------------------

using request_manager = grouped_coordinator;

// ---------------------------------------------------------------------------
// rx_request_t<BackendRequest> — per-reactor physical-request wrapper template.
// BackendRequest carries reactor-specific IO state (buffers, fd, etc.).
// ---------------------------------------------------------------------------

template <typename BackendRequest>
struct rx_request_t {
  BackendRequest inner;
};
