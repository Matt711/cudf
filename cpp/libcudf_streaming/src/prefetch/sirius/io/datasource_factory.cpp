...
cudf_streaming::prefetch::memory::fixed_size_host_memory_resource* first_host_resource(
  cudf_streaming::prefetch::memory::memory_reservation_manager& rm)
{
  auto host_spaces = rm.get_memory_spaces_for_tier(cudf_streaming::prefetch::memory::Tier::HOST);
  if (host_spaces.empty()) { return nullptr; }
  return host_spaces.front()->get_memory_resource_of<cudf_streaming::prefetch::memory::Tier::HOST>();
}
...
factory_type make_uring_ioctx_factory(
  cudf_streaming::prefetch::memory::memory_reservation_manager& reservation_manager)
{
...
io_context_registry::io_context_registry(
  config_type config, cudf_streaming::prefetch::memory::memory_reservation_manager& reservation_manager)
