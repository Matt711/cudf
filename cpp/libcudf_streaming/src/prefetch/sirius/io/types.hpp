...
namespace cudf_streaming::prefetch::io::cache {
class cached_chunk;
}
...
// ---------------------------------------------------------------------------
// io_object_segment — a single physical IO segment: an (offset, size) range
// within a file together with an optional contiguous or vectored buffer.
// Used by the io_uring reactor to describe one chunk of work.
// ---------------------------------------------------------------------------

struct io_object_segment {
  std::size_t offset{0};
  std::size_t size{0};

  io_object_segment() noexcept = default;
  io_object_segment(std::size_t off, std::size_t sz) noexcept : offset(off), size(sz) {}
  io_object_segment(std::size_t off, std::size_t sz, std::uint8_t* buf) noexcept
    : offset(off), size(sz), _data(buf)
  {
  }

  [[nodiscard]] std::uint8_t* data() const noexcept { return _data; }

  [[nodiscard]] bool is_vectored() const noexcept { return !_iovecs.empty(); }

  [[nodiscard]] bool is_buffer_allocated() const noexcept
  {
    return _data != nullptr || !_iovecs.empty();
  }

  void fill_remaining_buffers(std::size_t skip, std::vector<iovec>& out) const
  {
    out.clear();
    if (_data != nullptr) {
      if (skip < size) { out.push_back(iovec{_data + skip, size - skip}); }
      return;
    }
    std::size_t to_skip = skip;
    for (auto const& buf : _iovecs) {
      if (to_skip >= buf.iov_len) {
        to_skip -= buf.iov_len;
        continue;
      }
      auto* base = static_cast<std::uint8_t*>(buf.iov_base);
      out.push_back(iovec{base + to_skip, buf.iov_len - to_skip});
      to_skip = 0;
    }
  }
