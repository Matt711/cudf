...
#include "topology_discovery.hpp"
...
class topology_index {
 public:
  ...
  topology_index(cudf_streaming::prefetch::memory::system_topology_info topology,
  ...
  topology_index(cudf_streaming::prefetch::memory::system_topology_info topology,
                 const cudf_streaming::prefetch::memory::memory_reservation_manager& manager)
    : _topology(std::move(topology))
  {
    auto extract_ids = [](cudf_streaming::prefetch::memory::Tier tier) {
      return [tier](
               const cudf_streaming::prefetch::memory::memory_reservation_manager& manager) {
    ...
    _gpu_ids = extract_ids(cudf_streaming::prefetch::memory::Tier::GPU)(manager);
    std::vector<int> host_numa_nodes =
      extract_ids(cudf_streaming::prefetch::memory::Tier::HOST)(manager);
  ...
  [[nodiscard]] const cudf_streaming::prefetch::memory::system_topology_info& get_topology()
    const noexcept
  {
    return _topology;
  }
 ...
 private:
  cudf_streaming::prefetch::memory::system_topology_info _topology;
