...
{
  auto tracking_size = rmm::align_up(bytes, static_cast<std::size_t>(256));
  try {
                           ...
                           [[maybe_unused]] std::size_t alignment) noexcept
{
  ...
  auto* reservation_state       = _allocation_tracker->get_tracker_state(stream);
  if (reservation_state != nullptr) {
    auto* res          = reservation_state->memory_reservation.get();
    auto res_size      = static_cast<int64_t>(res->size());
    int64_t post_dealloc =
      res->allocated_bytes.sub(static_cast<int64_t>(tracking_bytes));
    int64_t pre_dealloc = post_dealloc + static_cast<int64_t>(tracking_bytes);
    if (pre_dealloc <= res_size) {
      upstream_reclaimed_bytes = 0;
    } else if (post_dealloc < res_size) {
      upstream_reclaimed_bytes = static_cast<std::size_t>(pre_dealloc - res_size);
    }
  }
...
//===----------------------------------------------------------------------===//
// reservation_aware_resource_adaptor (outer wrapper)
//===----------------------------------------------------------------------===//
