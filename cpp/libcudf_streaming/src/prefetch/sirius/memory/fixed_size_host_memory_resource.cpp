...
fixed_size_host_memory_resource::allocate_multiple_blocks(std::size_t total_bytes,
                                                          reservation* res)
{
  ...
  std::size_t total_bytes, chunked_reserved_area* h_reservation_slot)
{
  ...
  std::size_t num_blocks             = total_bytes / _block_size;
  ...
  std::size_t bytes, std::unique_ptr<event_notifier> on_release)
{
  ...
  if (bytes > _memory_limit) {
    throw std::runtime_error(
      "cudf_streaming::prefetch::memory::fixed_size_host_memory_resource: reservation size " +
...
bool fixed_size_host_memory_resource::grow_reservation_by(reserved_arena& arena,
                                                          std::size_t bytes)
{
...
{
  std::size_t reclaimed_bytes = chunks.size() * _block_size;
  std::lock_guard lock(_mutex);
