...
cdef class PackedColumns:
    ...
    """
    # def __cinit__(self, DeviceBuffer gd=None, vector[uint8_t] md=vector[uint8_t]()):
    #     with nogil:
    #         if gd is not None:
    #             self.c_obj.get().gpu_data = move(gd.c_obj)
    #         if md.size() > 0:
    #             self.c_obj.get().metadata = make_unique[vector[uint8_t]](move(md))
