...
def _leading_nulls_count(plc_col, size: int) -> int:
    ...
    total = plc.null_mask.null_count(bitmask, off, off + size)
    if total == 0:
        return 0
    if total == size:
        return size
    ...
    while lo + 1 < hi:
        mid = lo + ((hi - lo) // 2)
        cnt = plc.null_mask.null_count(bitmask, off, off + mid)
        if cnt == mid:
            lo = mid
        else:
            hi = mid
...
class ExponentialMovingWindow(_RollingBase):
    ...
    def _apply_agg_column(
        ...
        if leading_nulls == size:
            return to_libcudf_column
