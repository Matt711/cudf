...
def test_fused_scan_hybrid_multifile(
    tmp_path: Path,
    df: pl.DataFrame,
    predicate: pl.Expr,
    use_columns: list[str] | None,
    ...
    """FusedScan tasks (multiple physical files) dispatch to HybridScanMultifile."""
    streaming_engine = streaming_engine_factory(
        StreamingOptions(
            # Large enough that all files fuse into a single FusedScan task.
            target_partition_size=100_000_000,
            parquet_options={
                "use_hybrid_scan": True,
                "prefetch_file_metadata": True,
            },
    ...
    make_partitioned_source(df, tmp_path, "parquet", n_files=4, row_group_size=100)
