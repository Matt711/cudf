...
"""TPC-DS q52 — naive one-for-one Polars translation of the SQL."""
...
def polars_impl(run_config: RunConfig) -> QueryResult:
    """TPC-DS q52 naive Polars implementation."""
    params = load_parameters(
        int(run_config.scale_factor), query_id=52, qualification=run_config.qualification
    )
    year = params["year"]
    month = params["month"]
    ...
    result = result.sort(['d_year', 'ext_price', 'brand_id'], descending=[False, True, False], nulls_last=True).limit(100)
    return QueryResult(
        frame=result,
