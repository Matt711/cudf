...
"""TPC-DS q25 — naive one-for-one Polars translation of the SQL."""
...
def polars_impl(run_config: RunConfig) -> QueryResult:
    """TPC-DS q25 naive Polars implementation."""
    params = load_parameters(
        int(run_config.scale_factor), query_id=25, qualification=run_config.qualification
    )
    ...
    result = (
        ...
        .agg([
            pl.col("ss_net_profit").std(ddof=1).alias("store_sales_profit"),
            pl.col("sr_net_loss").std(ddof=1).alias("store_returns_loss"),
            pl.col("cs_net_profit").std(ddof=1).alias("catalog_sales_profit"),
        ])
    )
