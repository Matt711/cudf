...
"""TPC-DS q73 — naive one-for-one Polars translation of the SQL."""
...
def polars_impl(run_config: RunConfig) -> QueryResult:
    params = load_parameters(
        int(run_config.scale_factor), query_id=73, qualification=run_config.qualification
    )
    dom = params["dom"]
    bp = params["bp"]
    year = params["year"]
    ...
    #   AND d_dom BETWEEN dom[0] AND dom[1]
    #   AND (hd_buy_potential = bp[0] OR hd_buy_potential = bp[1])
    #   AND hd_vehicle_count > 0
    #   AND CASE WHEN hd_vehicle_count > 0 THEN hd_dep_count/hd_vehicle_count ELSE NULL END > 1
    #   AND d_year IN (year, year+1, year+2)
    #   AND s_county IN (counties)
    ...
    # Outer query:
    # FROM dj, customer WHERE ss_customer_sk = c_customer_sk AND cnt BETWEEN 1 AND 5
    result = (
        dj
    ...
    result = result.sort(["cnt", "c_last_name"], descending=[True, False], nulls_last=True)
    return QueryResult(
        frame=result,
