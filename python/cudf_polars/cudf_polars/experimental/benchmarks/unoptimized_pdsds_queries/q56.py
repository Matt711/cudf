...
"""TPC-DS q56 — naive one-for-one Polars translation of the SQL."""
...
def polars_impl(run_config: RunConfig) -> QueryResult:
    """TPC-DS q56 naive Polars implementation."""
    params = load_parameters(
        int(run_config.scale_factor), query_id=56, qualification=run_config.qualification
    )
    year = params["year"]
    month = params["month"]
    ...
    # Subquery item_ids: i_item_id IN (SELECT i_item_id FROM item WHERE i_color IN colors)
    color_item_ids = item.filter(pl.col("i_color").is_in(colors)).select("i_item_id").unique()

    # ss CTE:
    # FROM store_sales, date_dim, customer_address, item
    # WHERE i_item_id IN (color subquery)
    ...
    ss = (
        ...
        .join(color_item_ids, on="i_item_id", how="inner")
        .filter(
            (pl.col("d_year") == year)
            & (pl.col("d_moy") == month)
    ...
    # cs CTE (same structure, catalog_sales / cs_ prefix)
    cs = (
    ...
    # ws CTE (same structure, web_sales / ws_ prefix)
    ws = (
    ...
    # UNION ALL of ss, cs, ws then GROUP BY i_item_id, Sum(total_sales)
