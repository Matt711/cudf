...
def duckdb_impl(run_config: RunConfig) -> str:
    ...
    ORDER  BY total_cnt DESC,
              i_item_desc,
              w_warehouse_name,
              d1.d_week_seq --added prefix table alias to original query template
    LIMIT 100;
    """
...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Filter date dimensions more aggressively
    # d1: sold date (2002 only)
    d1_dates = (
    ...
    # Get the week sequences from 2002 to limit d2
    week_seqs_2002 = d1_dates.select("d1_week_seq").unique()
    # d2: inventory date (only weeks that exist in 2002)
    d2_dates = (
    ...
    # d3: ship date (only 2002 and nearby dates)
    d3_dates = (
        date_dim.filter(
            pl.col("d_year").is_not_null()
            & (pl.col("d_year").is_in([2001, 2002, 2003]))
        )  # Limit to nearby years
    ...
    # Start with filtered catalog_sales (2002 only) to reduce initial dataset size
    ...
    return (
        ...
        # LEFT OUTER JOIN with promotion
        .join(promotion, left_on="cs_promo_sk", right_on="p_promo_sk", how="left")
        # LEFT OUTER JOIN with catalog_returns
        .join(
            catalog_returns,
        ...
        # Group by item description, warehouse name, week sequence
