...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Subquery sa/sc: Revenue by store and item for 12-month period
    # (sa and sc are identical in the SQL, so we can reuse)
    ...
    # Subquery sb: Average revenue per store
    avg_revenue_by_store = revenue_by_store_item.group_by("ss_store_sk").agg(
        [pl.col("revenue").mean().alias("ave")]
    )
    # Main query: Join everything together and filter for low-revenue items
    return (
        ...
        .filter(
            # Find items where revenue <= 10% of store average
            pl.col("revenue") <= 0.1 * pl.col("ave")
        )
