...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # First query: store_sales customers
    store_customers = (
        store_sales.filter(
            pl.col("ss_sold_date_sk").is_not_null()
            & pl.col("ss_customer_sk").is_not_null()
        )
        # Join with date_dim (ss_sold_date_sk = d_date_sk AND d_month_seq between 1188 and 1199)
        .join(
            date_dim.filter(
    ...
    catalog_customers = (
        ...
        # Join with customer (cs_bill_customer_sk = c_customer_sk)
        .join(
            customer,
    ...
    # Third query: web_sales customers
    web_customers = (
        web_sales.filter(
            pl.col("ws_sold_date_sk").is_not_null()
            & pl.col("ws_bill_customer_sk").is_not_null()
        )
        # Join with date_dim (ws_sold_date_sk = d_date_sk AND d_month_seq between 1188 and 1199)
        .join(
            date_dim.filter(
                (pl.col("d_month_seq") >= 1188) & (pl.col("d_month_seq") <= 1188 + 11)
            ),
        ...
        # Join with customer (ws_bill_customer_sk = c_customer_sk)
        .join(
            customer,
    ...
    # Perform EXCEPT operations with NULL sentinel handling
    # Convert NULLs to sentinel values for proper anti-join matching
    ...
    # First EXCEPT: store_customers EXCEPT catalog_customers
    result_after_first_except = (
        store_customers_sentinel.join(
            catalog_customers_sentinel,
            on=["c_last_name", "c_first_name", "d_date"],
            how="anti",  # anti join = left except right
        ).unique()  # Ensure uniqueness after first EXCEPT
    )
    # Second EXCEPT: result EXCEPT web_customers
    result_after_second_except = (
        ...
        .unique()  # Ensure uniqueness after second EXCEPT
    )
