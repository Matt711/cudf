...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Create aliases for date_dim
    d1 = date_dim.clone().select(
        [
    ...
    d3 = date_dim.clone().select(
        [pl.col("d_date_sk").alias("d3_date_sk"), pl.col("d_year").alias("d3_year")]
    )
    return (
        ...
        #     (pl.col("d1_moy") == 4) &
        #     (pl.col("d1_year") == 1998) &
        #     (pl.col("d2_moy").is_between(4, 7)) &  # 4 + 3 = 7
        #     (pl.col("d2_year") == 1998) &
        #     (pl.col("d3_year").is_in([1998, 1999, 2000]))  # 1998, 1998+1, 1998+2
        # )
        .filter(
            (pl.col("d1_moy").is_not_null() & (pl.col("d1_moy") == 4))
            & (pl.col("d1_year").is_not_null() & (pl.col("d1_year") == 1998))
            & (
                pl.col("d2_moy").is_not_null() & pl.col("d2_moy").is_between(4, 7)
            )  # 4 + 3 = 7
            & (pl.col("d2_year").is_not_null() & (pl.col("d2_year") == 1998))
            & (
                pl.col("d3_year").is_not_null()
