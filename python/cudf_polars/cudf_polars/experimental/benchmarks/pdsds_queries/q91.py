...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # demo_filter = (
    #     ((pl.col("cd_marital_status") == "M") & (pl.col("cd_education_status") == "Unknown")) |
    #     ((pl.col("cd_marital_status") == "W") & (pl.col("cd_education_status") == "Advanced Degree"))
    # )
    demo_filter = (
    ...
    # Main query with all joins and filters
    return (
        catalog_returns
        # Join with call_center (cr_call_center_sk = cc_call_center_sk)
        .join(
            call_center,
        ...
        # Join with date_dim (cr_returned_date_sk = d_date_sk)
        .join(
            date_dim, left_on="cr_returned_date_sk", right_on="d_date_sk", how="inner"
        )
        # Join with customer (cr_returning_customer_sk = c_customer_sk)
        .join(
            customer,
        ...
        # Join with customer_demographics (cd_demo_sk = c_current_cdemo_sk)
        .join(
            customer_demographics,
        ...
        # Join with household_demographics (hd_demo_sk = c_current_hdemo_sk)
        .join(
            household_demographics,
        ...
        # Join with customer_address (ca_address_sk = c_current_addr_sk)
        .join(
            customer_address,
        ...
        .filter(
            ...
            & (pl.col("ca_gmt_offset").is_not_null() & (pl.col("ca_gmt_offset") == -7))
        )
        ...
        # Aggregate returns loss
        .agg([pl.col("cr_net_loss").sum().alias("sum(cr_net_loss)")])
        # Select final columns with proper aliases
        .select(
            [
        ...
        # Order by returns loss descending
