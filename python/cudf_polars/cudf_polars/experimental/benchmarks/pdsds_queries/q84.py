...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Build the query plan - following the SQL cross join logic
    return (
        customer
        # Join with customer_address (ca_city = 'Green Acres' AND c_current_addr_sk = ca_address_sk)
        .join(
        ...
        # Join with income_band (ib_income_band_sk = hd_income_band_sk AND income bounds)
        .join(
            income_band.filter(
        ...
        # Join with store_returns (sr_cdemo_sk = cd_demo_sk)
        # Note: cd_demo_sk is from customer_demographics, but after join we use the left_on key
        .join(
            store_returns,
            left_on="c_current_cdemo_sk",  # This equals cd_demo_sk from the earlier join
