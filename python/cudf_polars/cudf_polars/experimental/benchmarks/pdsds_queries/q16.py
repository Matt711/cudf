...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    exists_condition = (
        filtered_sales.with_row_index("row_id")  # Add unique row identifier
        .join(
    ...
    return (
        ...
        .join(
            exists_condition, on="row_id"
        )  # Only keep rows that satisfy EXISTS condition
        .join(
            returned_orders,
            left_on="cs_order_number",
            right_on="cr_order_number",
            how="anti",
        )  # Remove rows with returns
        .select(
            [
