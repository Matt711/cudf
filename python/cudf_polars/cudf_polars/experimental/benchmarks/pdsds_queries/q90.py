...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Base query with all joins and common filters
    base_query = (
        web_sales.join(
    ...
    # Count AM sales (12-13 hours) and PM sales (20-21 hours) in one pass
    return (
        base_query.select(
            [
                # AM count: t_hour between 12 and 13
                pl.when(pl.col("t_hour").is_between(12, 13))
                .then(1)
                .otherwise(0)
                .sum()
                .alias("amc"),
                # PM count: t_hour between 20 and 21
        ...
        .with_columns(
            [
                # Calculate AM/PM ratio with proper decimal casting and null handling
