...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Date range calculation: '2000-05-18' + 30 days = '2000-06-17'
    start_date = pl.date(2000, 5, 18)
    end_date = pl.date(2000, 6, 17)

    # Use explicit inner joins with how="inner" to match SQL comma-separated joins
    return (
        ...
        .with_columns(
            [
                # Calculate revenue ratio: itemrevenue * 100 / sum(itemrevenue) over class
                # This matches Sum(ss_ext_sales_price) * 100 / Sum(Sum(ss_ext_sales_price)) OVER (PARTITION BY i_class)
                (
                    pl.col("itemrevenue")
                    * 100.0
