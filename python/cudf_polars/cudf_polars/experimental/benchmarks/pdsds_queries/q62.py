...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Main query: Join all tables and calculate shipping delay buckets
    return (
        ...
        .with_columns(
            [
                ...
                # Create warehouse name substring
                pl.col("w_warehouse_name").str.slice(0, 20).alias("warehouse_substr"),
            ]
        )
        .with_columns(
            [
                # Preprocess: Create integer columns for each shipping delay bucket
                # 30 days or less
        ...
        .agg(
            [
                # Simple sum aggregations of preprocessed integer columns
        ...
        .sort(
            ["warehouse_substr", "sm_type", "web_name"],
            nulls_last=True,
