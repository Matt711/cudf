...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # CTE: ws (web sales without returns)
    ws = (
    ...
    # CTE: cs (catalog sales without returns)
    cs = (
    ...
    # CTE: ss (store sales without returns)
    ss = (
    ...
    # Main query: Join ss with ws and cs, calculate ratios
    return (
        ss.join(
            ws,
        ...
        .filter(
            (pl.coalesce([pl.col("ws_qty"), pl.lit(0)]) > 0)
            & (pl.coalesce([pl.col("cs_qty"), pl.lit(0)]) > 0)
        ...
        .select(
            [
                ...
                (
                    pl.col("ss_qty")
                    / pl.coalesce([pl.col("ws_qty") + pl.col("cs_qty"), pl.lit(1)])
                )
                .round(2)
                .alias("ratio"),
                ...
                (
                    pl.coalesce([pl.col("ws_qty"), pl.lit(0)])
                    + pl.coalesce([pl.col("cs_qty"), pl.lit(0)])
                ).alias("other_chan_qty"),
                (
                    pl.coalesce([pl.col("ws_wc"), pl.lit(0)])
                    + pl.coalesce([pl.col("cs_wc"), pl.lit(0)])
                ).alias("other_chan_wholesale_cost"),
                (
                    pl.coalesce([pl.col("ws_sp"), pl.lit(0)])
                    + pl.coalesce([pl.col("cs_sp"), pl.lit(0)])
