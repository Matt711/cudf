...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Common household demographics filter
    # hd_filter = (
    ...
    hd_filter = (
        (
            ...
            & pl.col("hd_vehicle_count").is_not_null()
            & (pl.col("hd_dep_count") == -1)
            & (pl.col("hd_vehicle_count") <= 1)
        )
        | (
            pl.col("hd_dep_count").is_not_null()
            & pl.col("hd_vehicle_count").is_not_null()
            & (pl.col("hd_dep_count") == 2)
            & (pl.col("hd_vehicle_count") <= 4)
        )
        | (
            pl.col("hd_dep_count").is_not_null()
            & pl.col("hd_vehicle_count").is_not_null()
            & (pl.col("hd_dep_count") == 3)
            & (pl.col("hd_vehicle_count") <= 5)
        )
    )
    # Base query for joining all tables with common filters
    base_query = (
        store_sales.join(
    ...
    # Count for each time period using conditional aggregation
    return base_query.select(
        [
            # h8_30_to_9: hour=8, minute>=30
            ...
            .cast(pl.Int64)
            .alias("h9_30_to_10"),
            # h10_to_10_30: hour=10, minute<30
            ...
            .cast(pl.Int64)
            .alias("h10_to_10_30"),
            # h10_30_to_11: hour=10, minute>=30
            pl.when((pl.col("t_hour") == 10) & (pl.col("t_minute") >= 30))
            .then(1)
            .otherwise(0)
            .sum()
            .cast(pl.Int64)
            .alias("h10_30_to_11"),
            # h11_to_11_30: hour=11, minute<30
            ...
            .cast(pl.Int64)
            .alias("h11_to_11_30"),
            # h11_30_to_12: hour=11, minute>=30
            pl.when((pl.col("t_hour") == 11) & (pl.col("t_minute") >= 30))
            .then(1)
            .otherwise(0)
            .sum()
            .cast(pl.Int64)
            .alias("h11_30_to_12"),
            # h12_to_12_30: hour=12, minute<30
