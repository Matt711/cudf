...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Create filtered date_dim views upfront to reduce join size
    date_2001 = date_dim.filter(pl.col("d_year") == 2001)
    date_2002 = date_dim.filter(pl.col("d_year") == 2002)

    # Helper function to create year totals for a specific sales table and year
    def create_year_total(
        ...
        return (
            customer.join(
                sales_table, left_on="c_customer_sk", right_on=customer_sk_col
            )
            .join(year_filter, left_on=date_sk_col, right_on="d_date_sk")
            .group_by(
                [
                    "c_customer_id",
                    "c_first_name",
    ...
    # Create the 4 specific year total views we need directly
    ...
    t_s_secyear = create_year_total(
        ...
        [
            ...
            pl.col("customer_birth_country").alias("customer_birth_country"),
