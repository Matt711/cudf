...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # CTE: ssci - Store sales customer-item combinations
    ssci = (
        store_sales.join(
        ...
        .agg([])  # Just grouping, no aggregations needed
        .select(
            [
    ...
    # CTE: csci - Catalog sales customer-item combinations
    csci = (
        catalog_sales.join(
    ...
    # Main query with FULL OUTER JOIN and CASE aggregations
    return (
        ssci.join(csci, on=["customer_sk", "item_sk"], how="full", suffix="_catalog")
        .select(
            [
                # Store only: ssci.customer_sk IS NOT NULL AND csci.customer_sk IS NULL
                pl.when(
                ...
                .sum()
                .alias("store_only"),
                # Catalog only: ssci.customer_sk IS NULL AND csci.customer_sk IS NOT NULL
                pl.when(
                ...
                .sum()
                .alias("catalog_only"),
                # Both: ssci.customer_sk IS NOT NULL AND csci.customer_sk IS NOT NULL
                pl.when(
                    pl.col("customer_sk").is_not_null()
                    & pl.col("customer_sk_catalog").is_not_null()
                )
