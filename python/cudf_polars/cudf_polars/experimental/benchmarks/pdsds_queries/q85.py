...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    return (
        web_sales
        # Join with web_returns (ws_item_sk = wr_item_sk AND ws_order_number = wr_order_number)
        .join(
            web_returns,
        ...
        # Join with web_page (ws_web_page_sk = wp_web_page_sk)
        .join(
            web_page, left_on="ws_web_page_sk", right_on="wp_web_page_sk", how="inner"
        )
        # Join with date_dim (ws_sold_date_sk = d_date_sk AND d_year = 2001)
        .join(
        ...
        # Join with customer_demographics cd1 (cd1.cd_demo_sk = wr_refunded_cdemo_sk)
        .join(
        ...
        # Join with customer_demographics cd2 (cd2.cd_demo_sk = wr_returning_cdemo_sk)
        .join(
            customer_demographics.select(
                [
        ...
        # Join with customer_address (ca_address_sk = wr_refunded_addr_sk)
        .join(
            customer_address,
        ...
        # Join with reason (r_reason_sk = wr_reason_sk)
        .join(reason, left_on="wr_reason_sk", right_on="r_reason_sk", how="inner")
        # Apply complex OR conditions
        .filter(
            # First OR condition: marital status and education matching with price ranges
            (
                # Condition 1: W + Primary + price 100-150
            ...
            | (
                # Condition 2: D + Secondary + price 50-100
            ...
            | (
                # Condition 3: M + Advanced Degree + price 150-200
        ...
        # Add the substr column for ordering
        .with_columns(
            [
