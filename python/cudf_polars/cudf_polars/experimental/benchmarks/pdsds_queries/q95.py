...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # Much more efficient: find orders with multiple warehouses using group_by
    ...
    # Find orders that have returns AND have multiple warehouses
    # No need for the complex ws_wh CTE - just check if returned orders are in multi_warehouse_orders
    ...
    return (
        ...
        # ws1.ws_order_number IN (SELECT wr_order_number FROM web_returns, ws_wh WHERE wr_order_number = ws_wh.ws_order_number)
        .join(
            returned_multi_warehouse_orders,
            left_on="ws_order_number",
