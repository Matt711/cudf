...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    # CTE 1: my_customers - Find customers who bought Sports/fitness items in May 2000
    # First, get the UNION of catalog and web sales
    ...
    my_customers = (
        ...
        .unique()  # DISTINCT equivalent
    )

    # Get the base month_seq for 2000-05
    _ = (
        date_dim.filter((pl.col("d_year") == 2000) & (pl.col("d_moy") == 5))
        .select(["d_month_seq"])
        .unique()
        .rename({"d_month_seq": "base_month_seq"})
    )

    # CTE 2: my_revenue - Calculate revenue for customers in following 3 months
    my_revenue = (
        my_customers.join(
        ...
        # TODO: GPU doesn't support NESTED LOOP JOIN
        # .join(base_month_seq, how="cross")
        # .filter(
        #     pl.col("d_month_seq").is_between(
        #         pl.col("base_month_seq") + 1, pl.col("base_month_seq") + 3
        #     )
        # )
    ...
    # CTE 3: segments - Create revenue buckets of $50
    segments = my_revenue.with_columns(
        [
            # Use rounding to match SQL CAST behavior exactly
            (pl.col("revenue") / 50.0).round(0).cast(pl.Int32).alias("segment")
        ]
    ).select("segment")
    # Final query: Count customers per segment
    return (
        segments.group_by("segment")
        .agg([pl.len().alias("count(*)")])
        .with_columns([(pl.col("segment") * 50).alias("segment_base")])
        .sort(["segment", "count(*)"], nulls_last=True, descending=[False, False])
        .limit(100)
        .select(
            [
                "segment",
                # Cast -> Int64 to match DuckDB
                pl.col("count(*)").cast(pl.Int64).alias("num_customers"),
                "segment_base",
            ]
        )
    )
