...
def _fuse_simple_reductions(
    ...
    for root_ir, group in reduction_groups.items():
        if len(group) > 1:
            ...
            # inner IR (select_b / children[0]) rather than the outer select_c.
            ...
            # final output names in select_c.schema.  Adding select_c itself to
            # the HConcat would expose only the final output name, causing a
            # KeyError when fused_select_c tries to look up the intermediate name.
